...
export class CallStack3D extends THREE.Group {
  ...
  private createBaseStructure(): void {
    ...
    for (let i = 0; i < 4; i++) {
      ...
      const beamGeometry = new THREE.CylinderGeometry(0.1, 0.1, this.options.maxFloors * (this.options.floorHeight + this.options.floorSpacing), 8);
  ...
  pushFunction(floorData: CallStackFloor): void {
    const availableFloor = this.floors.findIndex(floor => 
      !this.floorData.has(this.floors.indexOf(floor))
    );
