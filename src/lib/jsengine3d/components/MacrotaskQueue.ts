...
export class MacrotaskQueue {
  ...
  private createLabel(): void {
    ...
    ctx.fillText("MACROTASK QUEUE", canvas.width / 2, canvas.height / 2);
  ...
  public enqueue(content: string): void {
    ...
    // Create task (cube shape for timers/callbacks)
    const geometry = new THREE.BoxGeometry(this.taskSize, this.taskSize, this.taskSize);
  ...
  public update(): void {
    ...
    this.tasks.forEach((task, i) => {
      task.position.y = 0.8 + Math.sin(time * 1.5 + i * 0.5) * 0.08;
    });
  }
