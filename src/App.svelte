...
<main class="min-h-screen flex flex-col gap-4 p-4 md:p-8 max-w-6xl mx-auto">
  <header class="flex items-center justify-between">
    ...
    <nav class="flex items-center gap-3 text-sm text-gray-600 dark:text-gray-300" aria-label="Mode switcher">
      <label class="inline-flex items-center gap-2">
        <input type="checkbox" role="switch" aria-checked={mode === 'pro'} aria-label="Toggle Pro Mode" on:change={(e) => { mode = e.currentTarget.checked ? 'pro' : 'kid'; logs = [] }} />
  ...
  <aside class="grid gap-2 md:grid-cols-2" aria-label="Pro Mode Panels">
    <div class="rounded-lg border border-gray-200 dark:border-gray-800 p-3 bg-white dark:bg-gray-900">
      ...
      <div class="h-40 overflow-auto text-sm space-y-1" data-testid="logs">
        {#each logs as item}
          <div class="font-mono text-gray-700 dark:text-gray-300">{new Date(item.t).toLocaleTimeString()} — {item.msg}</div>
        {/each}
        {#if logs.length === 0}
